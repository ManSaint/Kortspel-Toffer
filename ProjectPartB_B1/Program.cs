...
namespace ProjectPartB_B1
{
    class Program
    {
        ...
        private static void HowManyCards()
        {
            Console.WriteLine("How many cards? 1-5");
            bool all_ok = int.TryParse(Console.ReadLine(), out int NrOfCards);
            while (!all_ok || NrOfCards < 1 || NrOfCards > 5)
            {
                Console.WriteLine("Invalid input. Please enter a value between 1 and 5.");
        ...
        {
            Console.WriteLine("How many rounds? 1-5");
            bool all_ok = int.TryParse(Console.ReadLine(), out int NrOfRounds);
